...
- 🟢 **Graceful JVM shutdown hook** - cancellation finalizer + JVM shutdown hook drain in-flight FlightSQL statements (up to `quack-on-demand.drainTimeoutSec`, default 60 s) and SIGTERM child Quack nodes via `LocalQuackBackend.cleanup()`. `docker-compose.yml` grows `stop_grace_period: 30s` so Docker gives the hook room before escalating to SIGKILL.
