...
Quack on Demand ships as a single uber-jar. Three installation paths are supported: download from Maven Central (the recommended default), build from source, and Docker. The native-jar path runs on **Linux, macOS, and Windows** - Windows uses PowerShell launchers (`*.ps1`) in place of the bash scripts; see [Run on Windows](#run-on-windows-native) below.
