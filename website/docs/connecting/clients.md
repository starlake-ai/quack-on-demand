...
The `tenant` and `pool` are passed as per-RPC call headers through `RPC_CALL_HEADER_PREFIX`. A superuser keeps both headers in place and adds `hdr + "superuser": "true"`. The bundled `scripts/loadtest/loadtest.py` is a ready-made ADBC client built exactly this way; pass `--superuser` (or `LT_SUPERUSER=true`) when running it as the bootstrap `admin` user so the same gRPC header is forwarded.
