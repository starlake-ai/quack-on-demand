...
export default function ScalePool() {
  ...
  useEffect(() => {
    ...
    api.poolStatus(tenant, tenantDb, pool).then(p => {
      setCurrent(p);
  ...
  async function submit(e: FormEvent) {
    ...
    try {
      await api.scalePool({
        tenant, tenantDb, pool, targetSize: target,
  ...
  const pEnc  = encodeURIComponent(pool!);
    ...
    <form onSubmit={submit}>
      <Breadcrumb
        items={[
          ...
          { label: tenant!,   to: `/tenant/${tEnc}` },
          { label: tenantDb! },
          { label: pool!,     to: `/pool/${tEnc}/${tdEnc}/${pEnc}` },
          { label: 'Scale' },
        ]}
      />
      <h2>Scale {tenant}/{tenantDb}/{pool}</h2>
