...
/** Per-statement classification of how a table is touched. Collapsed to three
  * values that line up with `ai.starlake.quack.model.StatementKind` and with
  * the production validator's grant check:
  *
  ...
  *
  * Granular verbs (SELECT/INSERT/UPDATE/DELETE individually) can be added
  * later by extending this enum; the validator's verb-match helper already
  * uses string-coalesced comparison so granular role-permission rows can be
  * mapped through. */
enum Verb:
  case Read, Write, Ddl

/** One (table, verb) tuple extracted from a statement. A single statement may
  * yield multiple tuples: e.g. `INSERT INTO t SELECT FROM s` produces
  * `TableAccess(t, Write) ++ TableAccess(s, Read)`. */
final case class TableAccess(table: TableRef, verb: Verb)

enum StatementResult:
  /** Statement was parsed AND we extracted its table access set.
    * `qualificationErrors` carries any DenyReasons accumulated while
    * resolving unqualified refs against the dialect-default catalog/schema. */
  case Extracted(
      index:               Int,
      sqlSnippet:          String,
  ...
  /** Statement parsed but carries no table references the validator cares
    * about: COMMIT, ROLLBACK, BEGIN, SET, USE, SHOW, EXPLAIN (without an
    * inner DML), etc. The validator admits these unconditionally. */
