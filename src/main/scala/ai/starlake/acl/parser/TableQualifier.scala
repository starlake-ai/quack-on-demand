...
/** Qualification pipeline: takes raw JSqlParser Table objects (output of TableExtractor), applies
  * DialectMapper to resolve partial names using Config defaults, and partitions into successfully
  * qualified TableRefs and DenyReason errors.
  */
object TableQualifier:
  ...
  /** Qualify a list of raw JSqlParser Table objects into TableRefs.
    *
    * @param tables
    *   raw Table objects from TableExtractor
    * @param config
    *   configuration with dialect and default database/schema
    * @return
    *   tuple of (successfully qualified TableRefs as Set, list of DenyReasons for failures)
    */
