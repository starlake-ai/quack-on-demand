...
case class AclConfig(
    @field @ConfigField(envVar = "QOD_ACL_ENABLED",
      description = "Enable table-level RBAC (per-statement EffectiveSet check).")
    enabled: Boolean,
    @field @ConfigField(envVar = "QOD_ACL_DIALECT",
...
/** Pre-statement SQL validation knobs. Loaded reflectively by the
  * config-page registry; not wired to runtime today (the legacy
  * file-store validator was retired with the RBAC cutover). Kept as
  * a typed class so the configurable env-var contract stays visible
  * in the admin UI. */
case class ValidationConfig(
    @field @ConfigField(envVar = "QOD_VALIDATION_ENABLED",
      description = "Enable per-statement SQL validation.")
    enabled: Boolean,
    @field @ConfigField(envVar = "QOD_VALIDATION_ALLOW_BY_DEFAULT",
      description = "When true, statements pass when no explicit rule matches.")
    allowByDefault: Boolean,
    @field @ConfigField(envVar = "QOD_VALIDATION_BYPASS_USERS",
...
object ValidationConfig:
  given ConfigReader[ValidationConfig] = ConfigReader.forProduct3(
    "enabled", "allowByDefault", "bypassUsers"
