...
case class DatabaseAuthConfig(
    @field @ConfigField(envVar = "QOD_AUTH_DB_ENABLED",
      description = "Enable database (bcrypt) authentication for the FlightSQL edge.")
    enabled: Boolean,
    @field @ConfigField(envVar = "QOD_AUTH_DB_JDBC_URL",
      description = "JDBC URL for the auth-lookup database.")
    jdbcUrl: String,
    @field @ConfigField(envVar = "QOD_AUTH_DB_USER",
      description = "Username for the auth-lookup JDBC connection.")
    username: String,
    @field @ConfigField(envVar = "QOD_AUTH_DB_PASSWORD",
      description = "Password for the auth-lookup JDBC connection.",
      sensitive = true)
    password: String,
    @field @ConfigField(envVar = "QOD_AUTH_DB_QUERY",
      description = "SQL template returning (password_hash, role); accepts three ? placeholders for tenant, pool, username.")
    query: String
) derives ConfigReader

case class KeycloakAuthConfig(
    @field @ConfigField(envVar = "QOD_AUTH_KEYCLOAK_ENABLED",
      description = "Enable the Keycloak OIDC bearer provider.")
    enabled: Boolean,
    @field @ConfigField(envVar = "QOD_AUTH_KEYCLOAK_BASE_URL",
      description = "Keycloak base URL (e.g. https://keycloak.example.com).")
    baseUrl: String,
    @field @ConfigField(envVar = "QOD_AUTH_KEYCLOAK_REALM",
      description = "Keycloak realm name.")
    realm: String,
    @field @ConfigField(envVar = "QOD_AUTH_KEYCLOAK_CLIENT_ID",
      description = "Keycloak client ID for ROPC.")
    clientId: String,
    @field @ConfigField(envVar = "QOD_AUTH_KEYCLOAK_CLIENT_SECRET",
...
case class GoogleAuthConfig(
    @field @ConfigField(envVar = "QOD_AUTH_GOOGLE_ENABLED",
      description = "Enable the Google OIDC bearer provider.")
    enabled: Boolean,
    @field @ConfigField(envVar = "QOD_AUTH_GOOGLE_CLIENT_ID",
      description = "Google OAuth client ID.")
    clientId: String,
    @field @ConfigField(envVar = "QOD_AUTH_GOOGLE_CLIENT_SECRET",
      description = "Google OAuth client secret.",
      sensitive = true)
    clientSecret: String,
    @field @ConfigField(envVar = "QOD_AUTH_GOOGLE_GROUPS_LOOKUP",
      description = "Resolve Google Workspace groups membership server-side.")
    groupsLookup: Boolean,
    @field @ConfigField(envVar = "QOD_AUTH_GOOGLE_SVC_ACCT_KEY_PATH",
      description = "Path to a Google service-account JSON used for groups lookup.")
    serviceAccountKeyPath: String,
    @field @ConfigField(envVar = "QOD_AUTH_GOOGLE_GROUPS_CACHE_TTL_SEC",
      description = "Cache TTL for Google groups lookups in seconds.")
    groupsCacheTtlSeconds: Long
) derives ConfigReader

case class AzureAuthConfig(
    @field @ConfigField(envVar = "QOD_AUTH_AZURE_ENABLED",
      description = "Enable the Azure AD bearer provider.")
    enabled: Boolean,
    @field @ConfigField(envVar = "QOD_AUTH_AZURE_TENANT_ID",
      description = "Azure AD tenant ID.")
    tenantId: String,
    @field @ConfigField(envVar = "QOD_AUTH_AZURE_CLIENT_ID",
      description = "Azure AD application (client) ID.")
    clientId: String,
    @field @ConfigField(envVar = "QOD_AUTH_AZURE_CLIENT_SECRET",
