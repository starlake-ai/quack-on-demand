...
/** Authenticates username/password credentials (database, ROPC).
  *
  * `tenant` scopes the lookup for the database backend: a tenant-scoped row identifies one
  * principal in that tenant; the wildcard NULL row matches across every tenant (superuser). OIDC
  * ROPC providers ignore it -- the OIDC server is authoritative.
  */
