...
object QuackNativeBridge:
  ...
  /** Serializes a `CONNECTION_REQUEST` message with the supplied auth token.
    * The returned bytes are the binary `QuackMessage` ready to POST to a
    * Quack node's `/quack` endpoint.
    */
  @native def serializeConnectionRequest(token: String): Array[Byte]

  /** Serializes a `PREPARE_REQUEST` message binding `sql` to an existing
    * connection identified by `connectionId`.
    */
  @native def serializePrepareRequest(connectionId: String, sql: String): Array[Byte]

  /** Serializes a `FETCH_REQUEST` message that asks the node to deliver
    * the next batch for the prepared result identified by `resultUuid`.
    *
    * `resultUuid` is the 128-bit identifier threaded through from the
    * `PREPARE_RESPONSE`; pass it as a `java.math.BigInteger` so the JNI
    * layer can unpack it into the native `hugeint_t {upper:int64, lower:uint64}`
    * pair without sign-mangling on the boundary.
    */
  ...
  /** Serializes a `DISCONNECT_MESSAGE` that releases server-side state for
  ...
  /** Parses `bytes` as a `CONNECTION_RESPONSE` and returns the
    * server-issued connection id. Raises `RuntimeException` if the
    * message is a different type.
    */
  @native def extractConnectionId(bytes: Array[Byte]): String

  /** Parses `bytes` as an `ERROR_RESPONSE` and returns the server's
    * human-readable error message. Raises `RuntimeException` if the
    * message is a different type.
    */
  @native def extractErrorMessage(bytes: Array[Byte]): String

  /** Parses `bytes` as a `PREPARE_RESPONSE` and returns its
    * `needs_more_fetch` flag. Only `PREPARE_RESPONSE` carries this flag
    * in the current submodule pin; passing a `FETCH_RESPONSE` raises a
    * `RuntimeException`.
    */
  ...
  /** Parses `bytes` as a `PREPARE_RESPONSE` or `FETCH_RESPONSE`,
    * transfers the embedded `DataChunkWrapper`s into a heap-allocated
    * Arrow C-data `ArrowArrayStream*`, and returns the raw pointer as a
    * `Long`. Ownership transfers to the caller: import it with
    * [[QuackArrowImport.importStream]], which delegates to
    * `org.apache.arrow.c.Data.importArrayStream`. The Arrow Java
    * importer invokes the stream's `release` callback when the returned
    * `ArrowReader` is closed, freeing the underlying chunks.
    *
    * Raises `RuntimeException` for any non-response message type or a
    * malformed buffer. Returns `0` if a Java exception is pending (the
    * caller must check `ExceptionCheck` semantics handled for you by
    * the JVM throwing on return).
    */
  @native def extractArrowStream(bytes: Array[Byte]): Long

  /** Parses `bytes` as a `FETCH_RESPONSE` and returns the number of
    * `DataChunkWrapper` entries it carries. The driver uses `0` as the
    * end-of-FETCH-loop signal, matching the upstream Quack-extension
    * scan code (`duckdb-quack/src/quack_scan.cpp:331`, which guards the
    * loop with `if (fetch_response->MutableResults().empty()) { ... return; }`).
    *
    * Raises `RuntimeException` if the message is a different type
    * (e.g. PREPARE_RESPONSE, which is handled by [[needsMoreFetch]] and
    * the initial Arrow stream).
    */
  @native def fetchResponseChunkCount(bytes: Array[Byte]): Int

  /** Parses `bytes` as a `PREPARE_RESPONSE` and returns the column names
    * declared by the server. Mirrors
    * `duckdb::PrepareResponseMessage::Names()`
    * (upstream `quack_message.hpp:157-159`, backing field
    * `result_names: vector<string>` at line 181). FETCH_RESPONSE carries
    * no schema and therefore no names; the driver pulls names once from
    * the PREPARE_RESPONSE and reuses them for every subsequent FETCH on
    * the same connection.
    *
