...
/** Opaque tag distinguishing serialized request bytes (manager -> node) from response bytes (node
  * -> manager). Keeps the wire direction visible at call sites in [[QuackProtocol]] (Task 6) so a
  * misuse (e.g. trying to decode a request as a response) becomes a type error.
  */
...
/** Mirror of duckdb-quack's `MessageType` enum (see `quack_message.hpp:9-21`). Ordinals are NOT
  * sequential -- the upstream pins specific numeric tags for wire compatibility, notably
  * `FETCH_REQUEST=7` (skipping 5 and 6) and `ERROR_RESPONSE=100`. When upstream changes the enum,
  * this file is the source of truth on the Scala side and must be updated to match.
  */
...
object MessageType:
  /** Resolve a wire ordinal back to a [[MessageType]]. Returns `None` for unrecognised ordinals so
    * callers can surface "unexpected message type N" rather than crashing. Note: this is keyed on
    * [[MessageType.wireOrdinal]] (the upstream tag), not Scala 3's synthetic `Enum.ordinal` (which
    * is the case's declaration index).
    */
