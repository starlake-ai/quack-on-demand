...
/** Decides how the FlightSQL `createPreparedStatement` action should obtain a `dataset_schema` for a
  * given SQL statement. The FlightSQL spec wants the Arrow schema of the future result *before* the
  * client calls `DoGet`; today's producer gets it by running the full statement and reading the
