...
/** One sub-group of nodes inside a pool that share a single node-scheduling [[NodePlacement]]. A
  * pool with `cohorts = Nil` schedules every node with no placement constraint (the legacy
  * behavior, equivalent to a single empty-placement cohort whose distribution equals the pool's
  * overall `RoleDistribution`).
  *
...
object PoolCohort:
  /** Synthesize a single placement-less cohort from a flat distribution. Used by the supervisor as
    * the fallback when a pool persists with `cohorts = Nil` (e.g. created via the legacy REST/YAML
    * path).
    */
