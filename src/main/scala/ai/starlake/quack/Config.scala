...
final case class RoleDistributionConfig(
    @field @ConfigField(envVar = "QOD_BOOTSTRAP_WRITEONLY",
      description = "Number of WRITEONLY nodes in the bootstrap pool.")
    writeonly: Int,
    @field @ConfigField(envVar = "QOD_BOOTSTRAP_READONLY",
      description = "Number of READONLY nodes in the bootstrap pool.")
    readonly: Int,
    @field @ConfigField(envVar = "QOD_BOOTSTRAP_DUAL",
...
final case class BootstrapConfig(
    ...
    enabled: Boolean,
    @field @ConfigField(envVar = "QOD_BOOTSTRAP_TENANT",
    ...
    @field @ConfigField(envVar = "QOD_BOOTSTRAP_TENANTDB",
      description = "Tenant-db suffix; actual DB is ${tenant}_${tenantDb}.")
    tenantDb: String,
    @field @ConfigField(envVar = "QOD_BOOTSTRAP_POOL",
      description = "Pool name seeded at boot.")
    pool: String,
    roleDistribution: RoleDistributionConfig
)

/** Typed view of the `quack-on-demand.defaultMetastore` block. Every
  * scalar maps to an env-var override the spawn-quack-node.sh contract
  * passes through to child nodes. `asMap` reproduces the legacy
  * `Map[String, String]` shape that backends + state stores still
  * consume. */
final case class DefaultMetastoreConfig(
    @field @ConfigField(envVar = "QOD_PG_HOST",
      description = "Postgres host for control plane + DuckLake catalog.")
    pgHost: String,
    @field @ConfigField(envVar = "QOD_PG_PORT",
      description = "Postgres port.")
    pgPort: String,
    @field @ConfigField(envVar = "QOD_PG_USER",
      description = "Postgres username used by the manager + Quack nodes.")
    pgUser: String,
    @field @ConfigField(envVar = "QOD_PG_PASSWORD",
      description = "Postgres password.",
      sensitive = true)
    pgPassword: String,
    @field @ConfigField(envVar = "QOD_PG_DBNAME",
      description = "Control-plane database name (default 'qod').")
    dbName: String,
    @field @ConfigField(envVar = "QOD_PG_SCHEMA",
      description = "Postgres schema for control-plane tables.")
    schemaName: String,
    @field @ConfigField(envVar = "QOD_DUCKLAKE_DATA_PATH",
...
final case class FlightConfig(
    @field @ConfigField(envVar = "PROXY_HOST",
      description = "FlightSQL edge bind address.")
    host: String,
    @field @ConfigField(envVar = "PROXY_PORT",
      description = "FlightSQL edge port.")
    port: Int,
    @field @ConfigField(envVar = "PROXY_TLS_ENABLED",
      description = "Enable TLS on the FlightSQL edge.")
    tlsEnabled: Boolean,
    @field @ConfigField(envVar = "PROXY_TLS_CERT_CHAIN",
      description = "Path to the TLS certificate chain PEM (auto-generated if missing).")
    tlsCertChain: String,
    @field @ConfigField(envVar = "PROXY_TLS_PRIVATE_KEY",
      description = "Path to the TLS private key PEM (auto-generated if missing).")
    tlsPrivateKey: String,
    @field @ConfigField(envVar = "PROXY_TENANT_CLAIM",
      description = "JWT claim that carries the tenant identifier for routing.")
    tenantClaim: String,
    @field @ConfigField(envVar = "QOD_SESSION_TTL_SEC",
