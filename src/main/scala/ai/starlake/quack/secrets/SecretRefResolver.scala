...
/** Resolves a prefixed secret reference (e.g. `env:GOOGLE_CLIENT_SECRET_TPCH`) to its plaintext
  * value. Same prefix vocabulary as `quack.ondemand.federation.DispatchingSecretResolver`
  * (`env:` / `aws-sm:` / `gcp-sm:` / `azure-kv:` / `vault:`), but stripped of the federation row
  * model so the edge can call it without dragging in `FederatedSecret`.
  *
  ...
  * `env:` path is enough to make per-tenant OIDC clientSecrets usable (operators inject the
  * secret as an env var on the manager process), and we don't need to lift four cloud-SDK
  * resolvers up the dependency graph before that vertical works end to end.
  */
...
object SecretRefResolver:
  ...
  /** Same as [[default]] but with an injectable env lookup -- the only seam tests need to drive
    * the resolver without polluting the process environment.
    */
  def fromEnv(envLookup: String => Option[String]): SecretRefResolver =
    (ref: String) =>
      ref.split(":", 2) match
        ...
        case Array(prefix, _)
            if Set("aws-sm", "gcp-sm", "azure-kv", "vault").contains(prefix) =>
          Left(
