...
/** Counter + timer for routed FlightSQL statements. One instance per manager; called from
  * FlightSqlRouter's post-process branch. The `status` label mirrors the same enum already written
  * to StatementHistoryStore so dashboards and history queries agree.
  *
  * The timer publishes a percentile histogram with SLO buckets at 1ms, 10ms, 100ms, 1s, 10s --
  * those become `_bucket` series in Prometheus and roll up cleanly in CloudWatch / Stackdriver /
  * Azure as count+sum statistics.
  */
final class StatementInstruments(registry: MeterRegistry):

  private val timerCache =
    new java.util.concurrent.ConcurrentHashMap[(String, String, String), Timer]()
  ...
  private def resolveTimer(tenant: String, pool: String, status: String): Timer =
    timerCache.computeIfAbsent(
      ...
      _ =>
        Timer
          .builder("statement_duration_seconds")
          .tag("tenant", tenant)
          .tag("pool", pool)
          .tag("status", status)
...
object StatementInstruments:
  ...
  /** Singleton no-op used when callers don't need real metric recording. Backed by an empty
    * `CompositeMeterRegistry` so `record(...)` is a silent fan-out to zero children. Allocated once
    * at class load.
    */
