...
/** Container for Micrometer registries. The `composite` is the single registry consumers
  * (instruments, gauges) write to -- Micrometer fans out to every attached child.
  * `prometheus.scrape()` produces the `/metrics` body when the Prometheus sink is active.
  */
final class MetricsRegistry(
    val composite: CompositeMeterRegistry,
    val prometheus: Option[PrometheusMeterRegistry]
):
  /** Closes all attached child registries. Each cloud child stops its push scheduler on close; the
    * composite stops accepting writes.
    */
  def close(): Unit =
    composite.close()
...
object MetricsRegistry extends LazyLogging:
  ...
  /** Build the registry from config. Returns a `Resource` so daemon threads spawned by cloud
    * registries (added in later tasks) shut down with the manager process.
    */
  def resource(cfg: MetricsConfig): Resource[IO, MetricsRegistry] =
    Resource.make(IO.delay(create(cfg)))(reg => IO.delay(reg.close()))

  /** A no-op registry -- used as a fallback (e.g. in tests) when no sink is needed. The composite
    * has no children, so counters/timers/gauges registered against it are silently discarded.
    */
