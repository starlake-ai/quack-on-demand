...
/** A single metrics sink: Prometheus pull, one of the cloud-monitoring pushes, or `Disabled` (no
  * sink). At most one is attached per process.
  *
  * `attach` returns the PrometheusMeterRegistry only when the sink is Prometheus -- that handle
  * lets `MetricsEndpoint` serve `/metrics`. All other sinks return `None`. Failures (missing
  * required config, init exceptions) are logged at WARN and yield `None`.
  */
...
private[metrics] object MetricsSink:
      ...
      try
        ...
        val awsCfg = new CloudWatchConfig:
          ...
          override def step(): java.time.Duration =
            java.time.Duration.ofSeconds(cfg.aws.stepSeconds)
        val client = CloudWatchAsyncClient.builder().build()
        val cw     =
          new CloudWatchMeterRegistry(awsCfg, io.micrometer.core.instrument.Clock.SYSTEM, client)
        composite.add(cw)
      catch
        case t: Throwable =>
        ...
        case Some(key) =>
          try
            ...
            val azCfg = new AzureMonitorConfig:
              ...
              override def step(): java.time.Duration   =
                java.time.Duration.ofSeconds(cfg.azure.stepSeconds)
            val az =
              new AzureMonitorMeterRegistry(azCfg, io.micrometer.core.instrument.Clock.SYSTEM)
            composite.add(az)
          catch
            case t: Throwable =>
              logger.warn(s"$name metrics disabled: ${t.getMessage}")
        ...
        case Some(project) =>
          try
            ...
            val sdCfg = new StackdriverConfig:
              ...
              override def step(): java.time.Duration =
                java.time.Duration.ofSeconds(cfg.gcp.stepSeconds)
  ...
  /** Look up the configured sink by name. Unknown values log a WARN and fall through to `Disabled`
    * so the manager doesn't crash.
    */
  def fromConfig(cfg: MetricsConfig, logger: Logger): MetricsSink =
    byName.getOrElse(
      cfg.sink, {
        logger.warn(
          s"unknown metrics.sink '${cfg.sink}'; expected one of ${byName.keys.toList.sorted.mkString(", ")}; falling back to 'none'"
        )
        Disabled
      }
    )
