...
/** In-memory mirror of the SCHEMA-bounded slice of the RBAC graph:
  * roles, groups, role permissions, group-role edges, and group-scoped
  * pool permissions. User-bound state (users themselves, user-role /
  * user-group / user-pool-permission edges) is deliberately NOT cached
  * here -- those live in Postgres and are fetched per-handshake by
  * [[ai.starlake.quack.ondemand.PoolSupervisor.effectiveSetForUser]].
  *
  * Memory footprint is bounded by tenant + schema cardinality (roles
  * per tenant, groups per tenant, permissions per role) rather than
  * org size, so the manager stays light even with millions of users.
  *
  * Reader methods expose the closure operations the spec calls
  * "rolesForGroup" and "permissionsForRoles". The handshake path
  * combines them with the per-user direct edges (read straight from
  * Postgres) to compute the spec's
  ...
  * }}}
  *
  * Superusers (`U.tenant IS NULL`) bypass effective_pools + effective_perms
  * upstream in the handshake; this resolver only answers the
  * tenant-scoped principal questions. */
final class RbacResolver:
  ...
  /** Replace the whole graph from a fresh snapshot (boot / reload).
    * Drops everything user-related from the snapshot -- this resolver
    * only mirrors the schema-bounded slice. */
  ...
  /** Cache the new grant ONLY if it's group-scoped. User-scoped grants
    * are looked up from Postgres on the handshake path. */
  ...
  /** All known roles, sorted by name. Used by the REST list endpoints
    * and the per-tenant breakdown filters. */
  ...
  def poolPermissionsForGroup(groupId: String): List[PoolPermission] =
    poolGrantsByGroup.getOrElse(groupId, Set.empty)
      .flatMap(poolPermissions.get).toList.sortBy(_.id)
