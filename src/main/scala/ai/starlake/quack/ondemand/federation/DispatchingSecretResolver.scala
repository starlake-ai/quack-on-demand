...
/** Routes a [[ai.starlake.quack.model.FederatedSecret]] to the right concrete resolver based on the
  * row's shape:
  *
  *   1. `value` is set -> [[PostgresSecretResolver]] returns the literal plaintext stored in
  *      `qodstate_federated_secret.value`.
  *   2. `externalRef` is set -> the prefix selects the resolver: `env:` `aws-sm:` `gcp-sm:`
  *      `azure-kv:` `vault:`.
  *
  * Lets a single manager mix backends: one source can hold its password in Vault while another
  * reads from AWS Secrets Manager, with no global mode flip needed. Wired by `Main.scala` when
  * `federation.secretStore = "dispatch"` (the default in newer deployments).
  *
  * Each leaf resolver must already be authenticated by its surrounding environment (SDK default
  * credential chain for the cloud backends, `VAULT_TOKEN` for Vault). The dispatcher itself holds
  * no credentials.
  */
  ...
  def resolve(secret: FederatedSecret): IO[String] =
    if secret.value.isDefined then postgres.resolve(secret)
      ...
      secret.externalRef match
        ...
        case None      =>
          IO.raiseError(
            new RuntimeException(s"secret '${secret.name}': neither value nor externalRef is set")
          )
