...
/** Full graph returned by [[ControlPlaneStore.snapshot]]. Loaded at
  ...
  *
  * The RBAC slice (users / roles / role permissions / groups /
  * membership edges / pool permissions) is included so
  * [[ai.starlake.quack.ondemand.rbac.RbacResolver]] can build its
  * in-memory caches in one trip. Empty by default so callers that only
  * care about the control-plane rows keep working. */
