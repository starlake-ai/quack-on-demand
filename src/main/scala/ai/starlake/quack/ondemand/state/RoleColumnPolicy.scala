...
/** A row in `qodstate_role_column_policy`. Attaches one column-level policy (deny or mask) on
  * `catalogName.schemaName.tableName.columnName` to a role. Catalog / schema / table accept `*` for
  * wildcards (same convention as [[RolePermission]]); `columnName` is always a literal.
  */
