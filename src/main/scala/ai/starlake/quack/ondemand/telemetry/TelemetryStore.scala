...
/** One audit event. Construction rejects detail keys that look like secrets so passwords,
  * tokens, and secret values can never be persisted by accident (audit-log spec section 5).
  */
...
/** Storage for telemetry-class data (audit events now; statement history and rollups are
  * added by the history-and-trends plan). Telemetry is append-heavy and time-partitioned
  * and may move to a different database class later, so: no foreign keys into qodstate_*
  * control-plane tables, identities stored as denormalized text, and no implementation
  * detail (SQL, cursors) leaks through this trait.
  */
