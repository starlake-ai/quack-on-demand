...
/** REST handlers for the membership surface (`/api/membership/...`).
  * Every endpoint is idempotent (the underlying store does ON CONFLICT
...
final class MembershipHandlers(sup: PoolSupervisor, mappers: UserHandlers):
  ...
  def addUserRole   (req: UserRoleMembershipRequest):  Out[Unit] = mapErr(sup.addUserRole   (req.userId,  req.roleId))
  def removeUserRole(req: UserRoleMembershipRequest):  Out[Unit] = mapErr(sup.removeUserRole(req.userId,  req.roleId))

  def addUserGroup   (req: UserGroupMembershipRequest): Out[Unit] = mapErr(sup.addUserGroup   (req.userId, req.groupId))
  def removeUserGroup(req: UserGroupMembershipRequest): Out[Unit] = mapErr(sup.removeUserGroup(req.userId, req.groupId))
