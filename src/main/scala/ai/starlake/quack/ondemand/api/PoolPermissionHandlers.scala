...
/** REST handlers for pool grants (`/api/pool/permission/...`). Resolves
  * `tenant` (display name OR id) the same way the role/group handlers
  * do. The grant principal is either `userId` or `groupId`; passing
  * both is a 400. */
