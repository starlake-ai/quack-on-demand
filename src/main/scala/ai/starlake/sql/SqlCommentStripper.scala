...
/** Removes single-line (`--`) and multi-line (`/* ... */`) SQL comments
  * from a query string. Single-quoted string literals are preserved
  * intact, including escaped quotes (`''`), so a literal like
  * `'-- not a comment'` is not stripped.
  *
  * Mirrors the implementation from the Starlake codebase
  * (`ai.starlake.sql.SqlCommentStripper`) so SQL-handling utilities stay
