...
/** Cross-tenant authZ on the RBAC REST surface.
  *
  * Before the fix, a tenant-A admin session could mutate tenant-B's RBAC graph
  * because none of the RBAC handlers consulted [[TenantScopeCheck]]. This spec
  * pins that down: every RBAC mutation called with a tenant-B id under a tenant-A
  * admin session must come back `403 tenant_forbidden`, while the same calls
  * under a superuser session (or static-key) must succeed.
  *
  * The fixture extends [[SecurityFixtures]] with a second tenant `globex`
  * (id `t-globex01`) carrying its own admin role + grant edge so the spec can
  * address tenant-B resources by id without per-test plumbing.
  */
class RbacTenantScopeSpec extends AnyFlatSpec with Matchers:
  ...
  /** Augment a [[SecurityFixtures.Fixture]] with a second tenant, a role + grant
    * inside it, a group, a user `carol` who has no admin grant, and one group-
    * scoped pool permission so [[revokePoolPermission]] has a target id.
    */
  ...
  private def bootWithTwoTenants(): (ManagerServerHarness.Harness, SecurityFixtures.Fixture, String) =
  ...
  "grantRolePermission" should "reject a tenant-A admin granting on a tenant-B role" in {
    ...
    try
      ...
      val resp  = post(h.httpClient, s"${h.baseUrl}/api/role/permission/grant", body, apiKey = Some(token))
  ...
  "revokeRolePermission" should "reject a tenant-A admin revoking a tenant-B permission" in {
    ...
    try
      ...
      val resp  = post(h.httpClient, s"${h.baseUrl}/api/role/permission/revoke", body, apiKey = Some(token))
  ...
  "addGroupRoleMembership" should "reject a tenant-A admin attaching to a tenant-B group" in {
    ...
    try
      ...
      val resp  = post(h.httpClient, s"${h.baseUrl}/api/membership/group-role/add", body, apiKey = Some(token))
  ...
  "effective" should "reject a tenant-A admin probing a tenant-B user's effective set" in {
    ...
    try
      val token = h.mintToken(SecurityFixtures.AliceUsername, SecurityFixtures.AlicePassword, Some(SecurityFixtures.TenantId))
      val resp  = get(h.httpClient, s"${h.baseUrl}/api/user/$carolId/effective", apiKey = Some(token))
  ...
  "revokePoolPermission" should "reject a tenant-A admin revoking a tenant-B grant" in {
    ...
    try
      ...
      val resp  = post(h.httpClient, s"${h.baseUrl}/api/pool/permission/revoke", body, apiKey = Some(token))
  ...
  "alice (tenant-A admin)" should "still be able to create + delete a role in her own tenant" in {
    ...
    try
      ...
      val createBody = s"""{"tenant":"${SecurityFixtures.TenantId}","name":"qa","description":"alice's role"}"""
      val create     = post(h.httpClient, s"${h.baseUrl}/api/role/create", createBody, apiKey = Some(token))
      ...
      val delResp = post(h.httpClient, s"${h.baseUrl}/api/role/delete", s"""{"id":"$newId"}""", apiKey = Some(token))
