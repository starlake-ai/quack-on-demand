...
class TokenRestrictionSpec extends AnyFlatSpec with Matchers:
  ...
  "narrow (branchOnly)" should "keep a parent's branch-only flag and let a child set it" in {
    val parent = r().copy(branchOnly = true)
    TokenRestriction.narrow(parent, U).map(_.branchOnly) shouldBe Right(true)
    TokenRestriction.narrow(U, U.copy(branchOnly = true)).map(_.branchOnly) shouldBe Right(true)
    TokenRestriction.narrow(U, U).map(_.branchOnly) shouldBe Right(false)
    U.branchOnly shouldBe false
  }
  ...
  it should "accept anything under ALL" in
    List("RO", "RW", "DDL", "ALL").foreach { v =>
      TokenRestriction
        .narrow(r(verbCeiling = Some("ALL")), r(verbCeiling = Some(v)))
  ...
  it should "let dropAdmin go false to true but never back" in {
    ...
    TokenRestriction
      .narrow(r(dropAdmin = true), r(dropAdmin = false))
      .map(_.dropAdmin) shouldBe Right(true)
  }

  it should "only let numeric limits decrease" in {
    TokenRestriction
      .narrow(r(maxRows = Some(100)), r(maxRows = Some(10)))
  ...
  it should "clamp a child expiry to the parent's" in {
    ...
    TokenRestriction
      .narrow(r(expiresAt = Some(early)), r(expiresAt = Some(late)))
      .map(_.expiresAt) shouldBe Right(Some(early))
    TokenRestriction
      .narrow(r(expiresAt = Some(late)), r(expiresAt = Some(early)))
      .map(_.expiresAt) shouldBe Right(Some(early))
