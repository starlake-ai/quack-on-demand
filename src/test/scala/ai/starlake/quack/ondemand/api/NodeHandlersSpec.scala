...
class NodeHandlersSpec extends AnyFlatSpec with Matchers:
  ...
  "quarantineNode" should "mark the node unhealthy" in:
    ...
    h.quarantineNode(NodeOpRequest("acme", "sales", nodeId)).unsafeRunSync() shouldBe Right(())
    tracker.snapshot(nodeId).healthy shouldBe false

  it should "return NotFound for unknown node" in:
    val (sup, _, h) = fixture
    val out = h.quarantineNode(NodeOpRequest("acme", "sales", "nope")).unsafeRunSync()
    out.left.toOption.map(_._1) shouldBe Some(StatusCode.NotFound)

  "setRole" should "reject unknown role" in:
    val (sup, _, h) = fixture
    val nodeId = sup.list().head.nodes.head.nodeId
    val out = h.setRole(SetRoleRequest("acme", "sales", nodeId, "NOPE")).unsafeRunSync()
    out.left.toOption.map(_._1) shouldBe Some(StatusCode.BadRequest)

  "setMaxConcurrent" should "update an existing node" in:
    val (sup, _, h) = fixture
    val nodeId = sup.list().head.nodes.head.nodeId
    val out = h.setMaxConcurrent(SetMaxConcurrentRequest("acme", "sales", nodeId, 7)).unsafeRunSync()
    out shouldBe Right(())
    sup.get(PoolKey("acme", "sales")).get.nodes.find(_.nodeId == nodeId).get.maxConcurrent shouldBe 7

  it should "reject negative max" in:
    val (sup, _, h) = fixture
    val nodeId = sup.list().head.nodes.head.nodeId
    val out = h.setMaxConcurrent(SetMaxConcurrentRequest("acme", "sales", nodeId, -1)).unsafeRunSync()
    out.left.toOption.map(_._1) shouldBe Some(StatusCode.BadRequest)

  it should "return NotFound for unknown node" in:
    val (sup, _, h) = fixture
    val out = h.setMaxConcurrent(SetMaxConcurrentRequest("acme", "sales", "nope", 5)).unsafeRunSync()
    out.left.toOption.map(_._1) shouldBe Some(StatusCode.NotFound)
