...
When the username itself is in email format, `email` is auto-set to it and immutable - `user/create` / `user/update` and manifest import reject a conflicting value with `400 invalid_email`; pre-existing rows are backfilled by Liquibase `0031-user-email-from-username`.
