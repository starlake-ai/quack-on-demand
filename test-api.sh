...
echo "==> status"
curl -fsS "$BASE/pool/acme/sales/status"
echo

echo "==> bump one node's maxConcurrent"
NODE_ID=$(curl -fsS "$BASE/pool/acme/sales/status" | python3 -c "import sys, json; print(json.load(sys.stdin)['nodes'][0]['nodeId'])")
curl -fsS -X POST "$BASE/node/setMaxConcurrent" -H 'Content-Type: application/json' -d "{
  \"tenant\":\"acme\",\"pool\":\"sales\",\"nodeId\":\"$NODE_ID\",\"max\":8
}"
echo

echo "==> scale to 3 (add 1 DUAL)"
curl -fsS -X POST "$BASE/pool/scale" -H 'Content-Type: application/json' -d '{
  "tenant":"acme","pool":"sales","targetSize":3,
...
curl -fsS -X POST "$BASE/pool/stop" -H 'Content-Type: application/json' -d '{
  "tenant":"acme","pool":"sales","force":true
}'
echo
echo "OK"